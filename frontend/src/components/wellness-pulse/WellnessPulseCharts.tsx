/**
 * Employee Wellness Pulse — Chart Visualizations
 *
...
import {
  WellnessTrend, DepartmentWellness, WellnessScore,
...
export const TrendLine: React.FC<{
  ...
  title: string;
  lines: { key: keyof WellnessTrend; color: string; label: string }[];
  yLabel?: string;
}> = ({ trends, title, lines, yLabel }) => {
  ...
  return (
    <div style={{ background: '#fff', borderRadius: 12, padding: 16, border: '1px solid #e5e7eb' }}>
      ...
      <svg width="100%" height={height} viewBox={`0 0 ${width} ${height}`}>
        {lines.map((line, li) => (
          <path key={li} d={toPath(trends.map(t => Number(t[line.key])))}
    ...
    <div style={{ background: '#fff', borderRadius: 12, padding: 16, border: '1px solid #e5e7eb' }}>
      ...
      {data.map((d, i) => (
        <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 6 }}>
          <span style={{ width: 90, fontSize: 11, color: '#6b7280', textAlign: 'right', flexShrink: 0 }}>{d.label}</span>
    ...
    <div style={{ background: '#fff', borderRadius: 12, padding: 16, border: '1px solid #e5e7eb', textAlign: 'center' }}>
      ...
      <svg width={size} height={size} viewBox={`0 0 ${size} ${size}`}>
        {[0.25, 0.5, 0.75, 1].map(s => (
          <polygon key={s}
            points={data.map((_, i) => pt(i, s)).map(p => `${p.x},${p.y}`).join(' ')}
